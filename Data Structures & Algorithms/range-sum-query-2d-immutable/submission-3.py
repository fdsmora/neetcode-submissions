...
class NumMatrix:
    ...
    def sumRegion(self, row1: int, col1: int, row2: int, col2: int) -> int:
        compensate = self.matrix[row1-1][col1-1] if row1>0 and col1>0 else 0
        upperSum = self.matrix[row1-1][col2] if row1>0 else 0
        leftSum = self.matrix[row2][col1-1] if col1>0 else 0
        return self.matrix[row2][col2]-upperSum-leftSum+compensate
